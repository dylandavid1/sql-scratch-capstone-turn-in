...
SELECT COUNT(DISTINCT user_id) AS '# of Users Who Made A Purchase'
FROM page_visits
WHERE page_name = '4 - purchase';

WITH last_touch AS (SELECT 
	user_id,
        MAX(timestamp) as last_touch_at
       FROM page_visits
       WHERE page_name = '4 - purchase'
...
SELECT lt_attr.utm_source AS 'Source',
       lt_attr.utm_campaign AS 'Campaign',
       COUNT(*) AS 'Last Touch Volume on Purchase page'
FROM lt_attr
GROUP BY 1, 2
ORDER BY 3 DESC;
